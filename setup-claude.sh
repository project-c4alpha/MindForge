...
# so that Claude Code can automatically discover your skills, commands and agents.
...
MINDFORGE_COMMANDS_DIR="$MINDFORGE_DIR/commands/$LANG_CODE"
...
if [ ! -d "$MINDFORGE_COMMANDS_DIR" ]; then
    echo -e "${YELLOW}Warning: Commands directory not found: $MINDFORGE_COMMANDS_DIR${NC}"
    echo "  Commands will be skipped."
fi
...
echo "  Commands: $MINDFORGE_COMMANDS_DIR"
echo "  Agents: $MINDFORGE_AGENTS_DIR"
echo "  Config: $MINDFORGE_DIR/config.toml.example"
...
if [ ! -f "$C4ALPHA_CONFIG" ]; then
    if [ -f "$MINDFORGE_DIR/config.toml.example" ]; then
        echo -e "${GREEN}Creating config file from template...${NC}"
        cp "$MINDFORGE_DIR/config.toml.example" "$C4ALPHA_CONFIG"
        echo -e "${GREEN}  Created: $C4ALPHA_CONFIG${NC}"
    else
        echo -e "${YELLOW}  Config template not found, skipping${NC}"
    fi
else
...
if [ -d "$MINDFORGE_COMMANDS_DIR" ]; then
    shopt -s nullglob
    COMMAND_FILES=("$MINDFORGE_COMMANDS_DIR"/*.md)
    shopt -u nullglob

    if [ ${#COMMAND_FILES[@]} -eq 0 ]; then
        echo -e "${YELLOW}No command files found in $MINDFORGE_COMMANDS_DIR${NC}"
    else
    ...
    echo -e "${YELLOW}Skipping commands (directory not found)${NC}"
fi
...
if [ -f "$C4ALPHA_CONFIG" ]; then
    ...
    echo -e "${YELLOW}  ○ $C4ALPHA_CONFIG not found (optional)${NC}"
fi
...
ls -d "$MINDFORGE_DIR/skills/"*/ 2>/dev/null | xargs -n 1 basename | sed 's/^/  - /'
echo ""
